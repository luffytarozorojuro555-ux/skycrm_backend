...
function getStatusAtDate(lead, toDate) {
  if (!lead.history || lead.history.length === 0) return null;

  let finalStatus = null;

  const sortedHistory = [...lead.history].sort(
    (a, b) => new Date(a.at) - new Date(b.at),
  );

  for (const h of sortedHistory) {
    if (new Date(h.at) <= toDate) {
      finalStatus = h.status?.name || h.status;
    } else {
      break;
    }
  }
...
export async function generateManagerReport(
  ...
  // ===== 1. Compute overall snapshot stats =====
  let totalLeads = 0;
  const totalStatusCount = {};

  teamsData.forEach((team) => {
    team.leadsAssigned?.forEach((lead) => {
      const status = getStatusAtDate(lead, toDate);
  ...
  const statsHTML = `
    <div class="section">
      <h2 class="section-heading">Overall Summary (As On Selected Date)</h2>
      <div class="stats-grid">
        <div class="stat-card">
          <p class="stat-title">Total Teams</p>
          <p class="stat-value">${teamsData.length}</p>
        ...
        <div class="stat-card">
          <p class="stat-title">Total Leads Across All Teams (As On Date)</p>
          <p class="stat-value">${totalLeads}</p>
        </div>
        <div class="stat-card">
          <p class="stat-title">Leads Converted (%)</p>
          <p class="stat-value">${overallProgress}%</p>
        </div>
      </div>
      <div class="status-breakdown">
        <h3>Lead Status As On Date</h3>
        <div class="status-grid">${statusBadgesHTML}</div>
      </div>
    </div>
  `;
  ...
  const teamsHTML = `
      ...
      ${teamsData
        .map((team) => {
          ...
          team.leadsAssigned?.forEach((lead) => {
            ...
            if (status) {
              teamTotalLeads++;
              teamStatusCount[status] = (teamStatusCount[status] || 0) + 1;
            }
          });

          const teamProgress = (
            ((teamStatusCount["Enrolled"] || 0) / (teamTotalLeads || 1)) *
            100
          ).toFixed(2);

          const teamStatusHTML = Object.entries(teamStatusCount)
            .map(
              ([status, count]) =>
    ...
    <div class="team-card">
      ...
      <div class="team-details">
        <p><strong>Members:</strong> ${team.members?.length || 0}</p>
        <p><strong>Leads (as of date):</strong> ${teamTotalLeads}</p>
        <p><strong>Conversion Rate (%):</strong> ${teamProgress}%</p>
      </div>
      <div class="team-status">
        <h4>Lead Status (As On Date)</h4>
        <div class="status-grid">${teamStatusHTML}</div>
      </div>
    </div>
  `;
...
<body>
<main>
<h1>Manager: ${managerData.name}</h1>
<div class="header-info">
<p>Email: ${managerData.email}</p>
<p><strong>Report period:</strong> ${formattedFrom} → ${formattedTo}</p>
<p><strong>Status as on: ${formattedTo}</strong></p>
</div>
${statsHTML}
${teamsHTML}
</main>
